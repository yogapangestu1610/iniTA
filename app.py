...
app.config['MAIL_USERNAME'] = 'ypangestu1610@gmail.com'
app.config['MAIL_PASSWORD'] = 'gucmdkvcjodqzaud'
...
#app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:@localhost/quizapp'
...
sender = 'ypangestu1610@gmail.com'
	...
	def wrap(*args, **kwargs):
		if 'logged_in' in session:
			if session['user_role']=="teacher":
				...
				flash('Anda tidak memiliki access untuk melihat Halaman ini')
				return render_template("404.html") 
		else:
			flash('Unauthorized, Silakan Login!')
...
def report_professor_email():
	if request.method == 'POST':
		careEmail = "ypangestu1610@gmail.com"
		cname = session['name']
		...
		mail.send(msg1)
		flash('Masalah anda telah direkam.', 'success')
	return render_template('report_professor.html')
...
def contact():
	if request.method == 'POST':
		...
		msg1.body = "Pesan Anda Telah Berhasil Terkirim"
		msg2 = Message('Hello', sender = sender, recipients = [careEmail])
...
def lostpassword():
	if request.method == 'POST':
		...
		if results > 0:
			...
			msg1 = Message('FraudCatcher - Verifikasi OTP Lupa Password', sender = sender, recipients = [lpemail])
			msg1.body = "Code OTP Verifikasi Lupa Password "+sesOTPfp+"."
			mail.send(msg1)
...
def lpnewpwd():
	if request.method == 'POST':
		...
		if(npwd == cpwd ):
			...
			cur.close()
			session.clear()
			return render_template('login.html',success="Password Berhasil diUbah.")
		else:
			return render_template('login.html',error="Password anda tidak sesuai.")
...
def register():
	if request.method == 'POST':
		...
		msg1 = Message('FraudCatcher - Kode OTP Verification', sender = sender, recipients = [email])
		msg1.body = "Membuaka Akun Baru - Kode OTP Verfication anda "+sesOTP+"."
		mail.send(msg1)
...
def verifyEmail():
	if request.method == 'POST':
		...
		if(theOTP == mOTP):
			...
			mysql.connection.commit()
			if ar > 0:
				flash("SELAMAT !!!. Registrasi Berhasil ! anda dapat login sekarang")
				return  redirect(url_for('login'))
			else:
				flash("Upps ! Terjadi Kesalahan")
				return  redirect(url_for('login')) 
			cur.close()
			session.clear()
		else:
			return render_template('register.html',error="Kode OTP anda salah.")
